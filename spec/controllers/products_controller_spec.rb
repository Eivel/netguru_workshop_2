...
describe ProductsController do
  ...
  let(:valid_attributes) { { "title" => "MyString", "category_id" => category.id, "description" => "MyString", "price" => "2.7" } }
  ...
  context "user is signed in" do #changes allowed after user logs in
    let!(:user) { create(:user) }
    let(:product) { Product.create!(valid_attributes) }
